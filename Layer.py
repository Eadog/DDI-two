...
class Attention(gnn.MessagePassing):
    ...
    def __init__(self, embed_dim, num_heads=8, dropout=0., bias=False,
        ...
        self.attn_sum = None
        config1 = MambaConfig(d_model=64, n_layers=2)
        self.Mamba = Mamba(config1).to('cuda:0')
    ...
    def forward(self,
        ...
        if self.se == 'khopgnn': # k-subgraph SAT
            ...
            # x_struct = self.Mamba(x_struct)
            # x_struct = x_struct.squeeze(1)
        ...
        return x_struct #torch.Size([12880, 256])
